...
w_2(x) := Q[w_1](x+c^*) = \int_{-\infty}^{\infty} k(y) g(w_1(x+c^*-y)) \, dy 
\end{equation} 
with $c^* \in \mathbb R$ a constant given by
\begin{equation}
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{fig:sub2}
\end{subfigure}
\caption{Plots of $w_1(x)$ (left) and $w_2(x)$ (right) with the Laplace dispersal kernel $k(x)=\frac{1}{2}e^{-|x|}$ and growth parameters $\alpha=0.3$, $\mu=0.5$, and $\beta=0.8$.}
\label{fig:test}
\end{figure}
