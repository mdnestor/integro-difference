...
\item \it{(Symmetry-preserving)} If $u(x-a)=u(x+a)$ then $Q[u](x-a)=Q[u](x+a)$.
\end{enumerate}
\end{prop}

 the cumulative density function $F(x)=\int_{-\infty}^{x}k(y)\,dy$ has a well-defined right inverse $F^{-1}$ satisfying $F(F^{-1}(p))=p$ for all $0<p<1$. Furthermore, it satisfies the symmetry relation $F(-x)=1-F(x)$.
...
\begin{proof} Let $x_\a=\sup\{x\in\mathbb R\mid w_1(x)\geq\a\}$ and $x_\b=\sup\{x\in\mathbb R\mid w_1(x)\geq\b\}$. It follows that $w_1(x)>\b$ for $x<x_\b$, $\a\leq w_1(x)\leq\b$ for $x_\b\leq x\leq x_\a$, and $w_1(x)<\a$ for $x>x_\a$. Applying the integro-difference operator to $w_1$ yields
...
\begin{proof}
It suffices to show that $u_{n+1}=Q[u_n]$ for all $n\geq 0$; this can be done via induction. The basis case is
\begin{equation}
u_1(x) = w_2(x) = Q[w_1](x) = Q[u_0](x)
\end{equation}
For the inductive step, suppose $u_{n+1}=Q[u_n]$ for some $n\geq 0$. We can proceed in cases. If $n$ is even, then $u_n(x)=w_1(x-nc^*)$ and $u_{n+1}(x)=w_2(x-nc^*)$; we have
\begin{equation}
u_{n+1}(x) = w_2(x-nc^*) = Q[w_1](x-nc^*) = Q[u_n](x)
\end{equation}
If $n$ is odd, then $u_n(x)=w_2(x-nc^*)$, and $u_{n+1}(x)=w_1(x-(n+1)c^*)$. By Lemma \ref{lemma1}, we have
\begin{equation}
u_{n+1}(x) = w_1(x-(n+1)c^*) = Q[w_2](x-nc^*) = Q[u_n](x)
\end{equation}
...
&= \limsup_{x\to\infty} \max\{w_1(x),w_2(x)\} &= 0.
\end{aligned} \end{equation}
Since the left hand side must also be non-negative, equality follows.
\end{proof}
...
\begin{thm} Let $u_0(x)$ be a non-negative continuous function with compact support such that the set $A=\{x\in\mathbb R:\a\leq u_0(x)\leq \b\}$ is connected. Suppose $k(x)$ has compact support, $\sup_{x\in\mathbb R}w_2(x)\leq \b$, and $c^* > 0$. Then, for $A$ sufficiently large, the sequence $(u_n)_{n=0}^{\infty}$ defined by $u_{n+1}=Q[u_n]$, $n\geq 0$ spreads with speed $c^*$, i.e., 
\begin{equation}
...
\begin{proof}
By the translation invariance property of $Q$, we may assume without loss of generality $A=[-r,r]$, for some $r>0$. It follows that $u_0(x)<\a$ for $|x|>r$ and $\a\leq u_0(x)\leq\b$ for $|x|$Furthermore, there exists $\sigma>0$ such that $k(x)>0$ if and only if $|x|<\sigma$. 
