...
If $\max_{x\in\mathbb R}w_2(x)\leq \b$, then there exists unique $c^*\in\mathbb R$ such that $Q[w_2](x)=w_1(x-2c^*)$ for all $x\in\mathbb R$.
\end{lem}

\begin{proof} Let $x_\a=\max\{x\in\mathbb R\mid w_1(x)\geq\a\}$ and $x_\b=\max\{x\in\mathbb R\mid w_1(x)\geq\b\}$. Since $w_1$ is monotone, it follows that $w_1(x)>\b$ for $x<x_\b$, $\a\leq w_1(x)\leq\b$ for $x_\b\leq x\leq x_\a$, and $w_1(x)<\a$ for $x>x_\a$. Applying the integro-difference operator to $w_1$ yields
...
\begin{equation}
c^*=\frac{1}{2}\max\{x\in\mathbb R\mid w_2(x)\geq\a\}
\end{equation}

It follows that $w_2(x)\geq\a$ for $x\leq 2c^*$ and $w_2(x)<\alpha$ for $x>2c^*$. Along with the assumption that $\max_{x\in\mathbb R}w_2(x)\leq\b$, we can apply the integro-difference operator once more:
...
The condition that $\max_{x\in\mathbb R}w_2(x)\leq \b$ may be difficult to verify for some choices of dispersal kernel. Thus, we provide a sufficient condition for this assumption:
...
then $\max_{x\in\mathbb R}w_2(x) \leq \b$.
\end{lem}

\begin{proof}
We have
\begin{equation} \begin{aligned}
w_2(x) &= (k*(g\circ w_1))(x) \\
&\leq (k*(m+(1-m)\mathbf 1_{[x_\b,x_a]}))(x) \\
&= m + (1-m)(k*\mathbf 1_{[x_\b,x_a]})(x) \\
...
\begin{equation} \begin{aligned}
\max_{x\in\mathbb R} w_2(x) &\leq \max_{x\in\mathbb R} \left( \m +  (1-\mu)\int_{x_\b}^{x_\a} k (x-y) \, dy \right) \\
&= \max_{x\in\mathbb R} \left( \m +  (1-\mu)\int_{-(x_\a-x_\b)/2}^{(x_\a-x_\b)/2} k (x-y) \, dy \right) \\
...
If $\max_{x\in\mathbb R}w_2(x)\leq \b$, then the recurrence \eqref{q} has a periodic traveling wave solution $(u_n)_{n=0}^{\infty}$ satisfying
...
\begin{figure}[h!] 
\centering
  \caption{The periodic traveling wave solution \eqref{ptw} with a monotone initial condition for times $1 \leq n \leq 11$ using the Laplace dispersal kernel, $\alpha=0.2$, $\mu=0.3$, and $\beta=0.8$. Later timepoints are colored in red, with earlier timepoints colored in green.}
\label{fig3}
...
If $\max_{x\in\mathbb R}w_2(x)\leq \b$, then for all $\epsilon>0$, the sequence \ref{ptw} satisfies
\begin{equation}
...
and 
\begin{equation}
\lim_{n\to\infty}\max_{x>n(c^*+\epsilon)}u_n(x)=0.
\end{equation}
\end{thm}
...
\begin{equation} \begin{aligned}
\lim_{n\to\infty} \max_{x>n(c^*+\epsilon)} u_n(x) 
&\leq \lim_{n\to\infty} \max_{x>n(c^*+\epsilon)} \max\{w_1(x-nc^*),w_2(x-nc^*)\} \\
&= \lim_{n\to\infty} \max_{x>n\epsilon} \max\{w_1(x),w_2(x)\} \\
...
\begin{thm} Let $u_0(x)$ be a non-negative continuous function with compact support such that the set $A=\{x\in\mathbb R:\a\leq u_0(x)\leq \b\}$ is connected. Suppose $k(x)$ has compact support, $\max_{x\in\mathbb R}w_2(x)\leq \b$, and $c^* > 0$. For $A$ sufficiently large, the sequence $(u_n)_{n=0}^{\infty}$ defined by $u_{n+1}=Q[u_n]$, $n\geq 0$ spreads with speed $c^*$, i.e.
\begin{equation}
